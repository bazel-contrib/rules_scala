#!/usr/bin/env bash
# Builds @io_bazel_rules_scala_guava//jar and checks whether its Maven source
# jar got fetched, based on the BAZEL_JVM_FETCH_SOURCES env var. The
...
# path and derive the source jar's path from it -- resolving the source jar's
...
srcjar_path="${main_jar_path%.jar}-src.jar"
