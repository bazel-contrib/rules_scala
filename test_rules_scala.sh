...
# The toolchain sweep below rebuilds the whole test/... tree under 2-3
# alternative --extra_toolchains values, to catch a tree-wide regression under
# those configs. Skipped on macOS/windows: ubuntu already covers it, saving
# ~19min/~9min per run respectively.
#
# Windows coverage of DependencyAnalyzer.scala's isWindows path-normalization
# branch, the one OS-specific piece of this logic, stays intact: that branch
# only runs when strict-deps or unused-deps checking is on, and dozens of
# per-target nested tests (test/missing_direct_deps, test/strict_dependency,
# test/plus_one_deps, etc.) already exercise those same toolchains through
# their own build_args, unconditionally, on every platform this script runs on.
